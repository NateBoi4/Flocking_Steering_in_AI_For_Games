...
Steering* ObstacleAvoidance::getSteering()
{
	...
	//GraphicsSystem::renderLine(*GraphicsSystem::getBackBuffer(), pOwner->getPositionComponent()->getPosition() + pOwner->getSprite().getCenterOffset(),
...
{
	if (ray.getX() > GraphicsSystem::getDisplayWidth()
		|| ray.getY() > GraphicsSystem::getDisplayHeight())
	{
		return true;
	}
	return false;
